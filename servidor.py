...
def dashboard():
    ...
    try:
        with get_db_connection() as conn:
            ...
            # Contar tareas pendientes sin asignar (Excluye PC Generica si ya fue asignada)
...
@app.route("/api/components/<string:serial_number>", methods=["GET"])
...
def add_component():
    ...
    try:
        ...
        with get_db_connection() as conn:
            conn.execute(
                "INSERT INTO components (serial_number, component_type, brand_model, status) VALUES (?, ?, ?, 'Stock')",
                (serial, ctype, model)
            )
            conn.commit()
...
@app.route("/api/components/assign", methods=["POST"])
def assign_component():
    """Asigna un componente de Stock a una PC."""
    try:
        data = request.json
        serial = data.get("serial_number")
        pc_name = data.get("pc_name")
        
        if not serial or not pc_name:
            return jsonify({"status": "error", "message": "Faltan datos"}), 400
        ...
        with get_db_connection() as conn:
            ...
            conn.execute(
                "UPDATE components SET status = 'Installed', assigned_pc = ? WHERE serial_number = ?",
                (pc_name, serial)
            )
            
            # 3. Registrar en Historial de PC
            detalles = f"{comp['component_type']} {comp['brand_model']} (S/N: {serial})"
            conn.execute(
                "INSERT INTO audit_logs (pc_name, field, old_value, new_value) VALUES (?, 'COMPONENT_ASSIGN', 'Stock', ?)",
                (pc_name, detalles)
            )
            conn.commit()
...
@app.route("/api/components/return", methods=["POST"])
def return_component():
    """Devuelve un componente instalado a Stock."""
    try:
        data = request.json
        serial = data.get("serial_number")
        
        if not serial:
            return jsonify({"status": "error", "message": "Falta serial"}), 400
        ...
        with get_db_connection() as conn:
            ...
            conn.execute(
                "UPDATE components SET status = 'Stock', assigned_pc = NULL WHERE serial_number = ?",
                (serial,)
            )
            
            # Registrar en Historial de PC (si tenia una asignada)
            if old_pc != "Unknown":
                detalles = f"{comp['component_type']} {comp['brand_model']} (S/N: {serial})"
                conn.execute(
                    "INSERT INTO audit_logs (pc_name, field, old_value, new_value) VALUES (?, 'COMPONENT_RETURN', ?, 'Stock')",
                    (old_pc, detalles)
                )
            conn.commit()
