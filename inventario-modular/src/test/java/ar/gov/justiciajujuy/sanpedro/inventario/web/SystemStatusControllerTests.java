...
class SystemStatusControllerTests {
	...
	@Test
	void protectsRuntimeSystemStatusBehindAuthentication() throws Exception {
		mockMvc.perform(get("/api/v1/sistema/runtime"))
			.andExpect(status().isUnauthorized());
	}

	@Test
	void exposesRuntimeSystemStatusOnlyForAuthenticatedUsers() throws Exception {
		mockMvc.perform(get("/api/v1/sistema/runtime").with(user(adminLocal())))
			.andExpect(status().isOk())
	...
	private ActiveDirectoryUserDetails adminLocal() {
		return new ActiveDirectoryUserDetails(
				...
				"Administrador Local",
				"Desarrollo local",
				java.util.Map.of("origen", List.of("LOCAL_SIMULADO")));
	}
}
