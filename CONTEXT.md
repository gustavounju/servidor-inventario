...
- **UI/UX & SQL (12 Julio 2026)**: Refactorización profunda del layout y las tablas del Visor de Tareas (`visor_tareas.html`). Se rediseñó el panel superior eliminando texto redundante (ej. "COMPLETADAS 0 Trabajos Resueltos Hoy") a favor de tarjetas de métricas más compactas e impactantes. En las tablas de datos, se removió la columna 'SOLICITANTE' unificando la identificación en la columna 'EQUIPO/USUARIO/FUERO' con tamaño fijo (240px) para priorizar el espacio de la descripción del trabajo. Asimismo, se corrigió un solapamiento crítico de columnas en SQL (`bp_tasks.py`) usando el alias `pc_fuero` para evitar que la base de datos sobrescriba el fuero real de la PC con la columna vacía de las tareas en los `LEFT JOIN`.
---
