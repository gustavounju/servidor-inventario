...
def run_all_migrations():
    ...
    migrate_db_v55()
    # Simplificación operativa de estados patrimoniales
    migrate_db_v56()
    # Ignorar falsos positivos en Auto-ID duplicados
    migrate_db_v57()
...
def migrate_db_v48():
    """Migración V48: Marcar is_autogenerated_id = 1 para componentes cuyo serial_number inicie con INT-."""
    print("Verificando migración de DB v48 (Flag Auto ID en componentes)...")
    with get_db_connection() as conn:
        if _table_exists(conn, "components"):
            conn.execute("UPDATE components SET is_autogenerated_id = 1 WHERE serial_number LIKE 'INT-%%' AND (is_autogenerated_id IS NULL OR is_autogenerated_id = 0)")
...
def migrate_db_v55():
    ...
    y build_order_id = bo.id si la orden está en borrador/en armado, o 'Installed'
    ...
    with get_db_connection() as conn:
        ...
        if False:
            ...
            try:
                ...
                # 2. Órdenes completadas -> status = 'Installed'
                conn.execute(
                    """
                    UPDATE components c
                    JOIN build_order_items boi ON c.serial_number = boi.serial_number
                    JOIN build_orders bo ON boi.build_order_id = bo.id
                    SET c.status = 'Installed',
                        c.build_order_id = bo.id,
                        c.assigned_pc = COALESCE(NULLIF(boi.pc_name, ''), NULLIF(bo.target_pc_name, ''), c.assigned_pc),
                        c.assigned_user = COALESCE(NULLIF(bo.target_user, ''), c.assigned_user),
                        c.assigned_fuero = COALESCE(NULLIF(bo.target_fuero, ''), c.assigned_fuero)
...
def migrate_db_v56():
    """
    Migración V56: Simplificar estados operativos de components.

    Normaliza el modelo vigente del negocio:
      - 'Asignado'     -> 'Installed'
      - 'Stock (Combo)' -> 'Stock'

    Además alinea lifecycle_status para evitar lecturas ambiguas entre
    el estado operativo y el estado patrimonial.
    """
    print("Verificando migración de DB v56 (normalización de estados operativos)...")
    with get_db_connection() as conn:
        if not _table_exists(conn, "components"):
            print("Migración V56: tabla components no encontrada. Skip.")
            return
        ...
        conn.execute(
            """
            UPDATE components
            SET lifecycle_status = 'desplegado'
            WHERE TRIM(status) = 'Installed'
              AND (lifecycle_status IS NULL OR TRIM(lifecycle_status) = '' OR TRIM(lifecycle_status) = 'en_armado')
            """
        )
        conn.execute(
            """
            UPDATE components
            SET lifecycle_status = 'stock'
            WHERE TRIM(status) = 'Stock'
              AND (lifecycle_status IS NULL OR TRIM(lifecycle_status) = '' OR TRIM(lifecycle_status) = 'en_armado')
            """
        )

    print("Migración V56 verificada.")


def migrate_db_v57():
    """
    Migración V57: Permitir ignorar falsos positivos en Auto-ID duplicados.

    Agrega un flag persistente sobre components para que un componente
    autogenerado INT-* pueda quedar excluido del tablero de limpieza
    cuando el equipo de patrimonio decide conservarlo.
    """
    print("Verificando migración de DB v57 (ignorar Auto-ID duplicados)...")
    with get_db_connection() as conn:
        if not _table_exists(conn, "components"):
            print("Migración V57: tabla components no encontrada. Skip.")
            return

        if not _column_exists(conn, "components", "ignore_autogenerated_duplicate"):
            print("Aplicando V57: agregando ignore_autogenerated_duplicate...")
            conn.execute(
                "ALTER TABLE components ADD COLUMN ignore_autogenerated_duplicate TINYINT(1) NOT NULL DEFAULT 0"
            )

    print("Migración V57 verificada.")


def migrate_db_v58():
    """
    Migración V58: Registrar quién y cuándo ignoró un Auto-ID duplicado.

    Agrega metadata operativa sobre components para mostrar trazabilidad
    directa en la UI de Gestión de Patrimonio y Remitos.
    """
    print("Verificando migración de DB v58 (trazabilidad de Auto-ID ignorados)...")
    with get_db_connection() as conn:
        if not _table_exists(conn, "components"):
            print("Migración V58: tabla components no encontrada. Skip.")
            return

        if not _column_exists(conn, "components", "ignored_autogenerated_by"):
            print("Aplicando V58: agregando ignored_autogenerated_by...")
            conn.execute(
                "ALTER TABLE components ADD COLUMN ignored_autogenerated_by VARCHAR(255) DEFAULT NULL"
            )

        if not _column_exists(conn, "components", "ignored_autogenerated_at"):
            print("Aplicando V58: agregando ignored_autogenerated_at...")
            conn.execute(
                "ALTER TABLE components ADD COLUMN ignored_autogenerated_at DATETIME DEFAULT NULL"
            )
