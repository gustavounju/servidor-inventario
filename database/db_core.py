...
def init_db():
    ...
    with get_db_connection() as conn:
        ...
        # Migraciones automáticas para la tabla components
        comp_migrations = [
            ("assigned_fuero", "VARCHAR(255) DEFAULT NULL AFTER assigned_to_component_id"),
            ("assigned_user", "VARCHAR(255) DEFAULT NULL AFTER assigned_fuero"),
            ("oc_number", "VARCHAR(100) DEFAULT NULL AFTER invoice_number"),
            ("is_autogenerated_id", "TINYINT(1) DEFAULT 0 AFTER oc_number"),
            ("kit_name", "VARCHAR(255) DEFAULT NULL AFTER is_autogenerated_id")
        ]
        for col, col_def in comp_migrations:
            try:
                conn.execute(f"ALTER TABLE components ADD COLUMN {col} {col_def}")
                print(f"Migración: Columna '{col}' añadida a components exitosamente.")
            except Exception as e:
                logging.debug(f"Columna {col} en components ya existe o error menor: {e}")

        # Intentar crear índices en components
        indices_comp = [
            ("idx_comp_assigned_user", "CREATE INDEX idx_comp_assigned_user ON components(assigned_user)"),
            ("idx_comp_assigned_fuero", "CREATE INDEX idx_comp_assigned_fuero ON components(assigned_fuero)"),
            ("idx_comp_invoice_number", "CREATE INDEX idx_comp_invoice_number ON components(invoice_number(100))"),
            ("idx_comp_kit_name", "CREATE INDEX idx_comp_kit_name ON components(kit_name)")
        ]
        ...
        # Intentar añadir columna can_manage_stock si no existe
        try:
            conn.execute("ALTER TABLE app_users ADD COLUMN can_manage_stock TINYINT(1) DEFAULT 0 AFTER can_audit_racks")
            print("Migración: Columna 'can_manage_stock' añadida a app_users exitosamente.")
        except Exception as e:
            logging.debug(f"Columna can_manage_stock ya existe o error menor: {e}")
