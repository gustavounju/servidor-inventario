...
def homologar_telemetria():
    ...
    try:
        with get_db_connection() as conn:
            pc_row = conn.execute(
                "SELECT telemetry_snapshot, full_json_data, processor, ram_gb, motherboard_model, disk_models, last_user, fuero FROM pcs WHERE pc_name = %s",
                (pc_name,)
            ).fetchone()
            ...
            # Insertar activos en la tabla components para que figuren en Patrimonio y Remitos
            assigned_user = (pc_row.get("last_user") or "").strip() or None
            assigned_fuero = (pc_row.get("fuero") or "").strip() or None

            # Buscar si existe una Orden de Armado vinculada a este equipo o usuario
            linked_bo = conn.execute(
                """
                SELECT id, code FROM build_orders
                WHERE LOWER(TRIM(target_pc_name)) = LOWER(TRIM(%s))
                   OR (target_user IS NOT NULL AND %s IS NOT NULL AND LOWER(TRIM(target_user)) = LOWER(TRIM(%s)))
                ORDER BY created_at DESC LIMIT 1
                """,
                (pc_name, assigned_user or "", assigned_user or "")
            ).fetchone()

            bo_id = linked_bo["id"] if linked_bo else None

            def _ensure_homologated_component(comp_type, brand_model, raw_serial=None):
                ...
                query_params = [pc_name]
                if bo_id:
                    query_parts.append("(build_order_id = %s)")
                    query_params.append(bo_id)
                ...
                existing = conn.execute(
                    ...
                    LIMIT 1
                    """,
                    (*query_params, comp_type)
                ).fetchone()

                if not existing:
                    from blueprints.bp_stock import generate_internal_serial
                    sn = (raw_serial or "").strip()
                    is_auto = 0
                    if not sn or sn.upper() in ("N/A", "UNKNOWN", "TO BE FILLED BY O.E.M.", "DEFAULT STRING", "00000000"):
                        sn = generate_internal_serial(comp_type)
                        is_auto = 1

                    dup = conn.execute("SELECT id FROM components WHERE serial_number = %s", (sn,)).fetchone()
                    if dup:
                        sn = generate_internal_serial(comp_type)
                        is_auto = 1
                    ...
                    try:
                        conn.execute(
                            ...
                            INSERT INTO components (
                                ...
                                assigned_pc, assigned_user, assigned_fuero, build_order_id, supplier,
                                invoice_number, oc_number, is_autogenerated_id, created_at
                            ) VALUES (
                                %s, %s, %s, 'Asignado', 'desplegado',
                                %s, %s, %s, %s, 'Homologación Telemetría WMI',
                                NULL, NULL, %s, NOW()
                            )
                            """,
                            (sn, comp_type, brand_model_str, pc_name, assigned_user, assigned_fuero, bo_id, is_auto)
                        )

                        if bo_id:
                            boi_dup = conn.execute("SELECT id FROM build_order_items WHERE build_order_id = %s AND serial_number = %s", (bo_id, sn)).fetchone()
                            if not boi_dup:
                                conn.execute(
                                    """
                                    INSERT INTO build_order_items (build_order_id, serial_number, asset_type, brand_model, pc_name, scanned_at, scanned_by)
                                    VALUES (%s, %s, %s, %s, %s, NOW(), 'Homologación WMI')
                                    """,
                                    (bo_id, sn, comp_type, brand_model_str, pc_name)
                                )
                    except Exception as ins_err:
                        logging.warning("Error creando componente %s para %s: %s", comp_type, pc_name, ins_err)

            # 1. Motherboard
            if new_mb:
                m_sn = re.search(r"\(([A-Za-z0-9\-]+)\)", str(new_mb))
                mb_sn = m_sn.group(1) if m_sn else None
                _ensure_homologated_component("Placa Madre", new_mb, mb_sn)

            # 2. Procesador
            if new_proc:
                _ensure_homologated_component("Procesador", new_proc)

            # 3. Memoria RAM
            if new_ram:
                ram_str = f"{new_ram} GB RAM" if isinstance(new_ram, (int, float)) or str(new_ram).replace('.','',1).isdigit() else str(new_ram)
                _ensure_homologated_component("Memoria RAM", ram_str)
            ...
            if new_disks:
                ...
                for d in disks:
                    ...
                    _ensure_homologated_component("Disco Rígido / SSD", d, sn_val)
