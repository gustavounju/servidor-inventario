...
def stock_view():
    try:
        with get_db_connection() as conn:
            ...
            stock_comps_rows = conn.execute(
                ...
                FROM components
                WHERE status = 'Stock' OR status IS NULL OR assigned_pc IS NULL OR assigned_pc = ''
    ...
    return render_template("stock.html", pcs=pcs, ad_users=ad_users, stock_components=stock_components)
