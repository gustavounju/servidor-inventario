...
from utils.component_status import assignment_component_state, deployed_component_state, retired_component_state, stock_component_state
...
def _load_autogenerated_duplicate_candidates(conn):
    rows = conn.execute(
        """
        SELECT *
        FROM components
        WHERE (assigned_pc IS NOT NULL AND TRIM(assigned_pc) != '')
           OR build_order_id IS NOT NULL
           OR (assigned_user IS NOT NULL AND TRIM(assigned_user) != '')
           OR (assigned_fuero IS NOT NULL AND TRIM(assigned_fuero) != '')
        ORDER BY assigned_pc ASC, component_type ASC, brand_model ASC
        """
    ).fetchall()
    items = []
    for row in rows:
        ...
        item["is_autogenerated_id"] = 1 if item.get("is_autogenerated_id") or serial.upper().startswith("INT-") else 0
        items.append(item)
    return find_autogenerated_shadow_components_by_owner(items)


def _load_ignored_autogenerated_duplicates(conn):
    rows = conn.execute(
        """
        SELECT *
        FROM components
        WHERE ignore_autogenerated_duplicate = 1
        ORDER BY assigned_pc ASC, component_type ASC, brand_model ASC
        """
    ).fetchall()
    items = []
...
def stock_view():
    initial_stock_filters = {
        ...
        "remito": (request.args.get("remito") or "").strip(),
        "supplier": (request.args.get("supplier") or "").strip(),
        "open_modal": (request.args.get("open_modal") or "").strip(),
    }
    try:
        with get_db_connection() as conn:
            ...
            try:
                autogenerated_duplicate_candidates = _load_autogenerated_duplicate_candidates(conn)
            except Exception:
                autogenerated_duplicate_candidates = []
    ...
    return render_template(
        ...
        autogenerated_duplicate_candidates=autogenerated_duplicate_candidates,
        initial_stock_filters=initial_stock_filters,
    )


@bp_stock.route("/api/components/resolve_autogenerated_duplicates", methods=["POST"])
def resolve_autogenerated_duplicates():
    if not check_stock_permission():
    ...
    data = request.json or {}
    requested_serials = [str(s).strip() for s in (data.get("serials") or []) if str(s).strip()]

    with get_db_connection() as conn:
        candidates = _load_autogenerated_duplicate_candidates(conn)
        candidate_map = {c.get("serial_number"): c for c in candidates if c.get("serial_number")}

        if requested_serials:
            serials_to_delete = [serial for serial in requested_serials if serial in candidate_map]
        else:
            serials_to_delete = list(candidate_map.keys())
        ...
        if not serials_to_delete:
            return jsonify({
                ...
                "deleted_count": 0,
                "deleted_serials": [],
                "message": "No se detectaron componentes autogenerados duplicados válidos para resolver.",
            })

        actor = current_technician_identity() or current_username() or "Sistema"
        deleted_serials = []
        for serial in serials_to_delete:
            component = candidate_map[serial]
            old_pc = component.get("assigned_pc") or component.get("assigned_user") or component.get("assigned_fuero") or "Stock"
    ...
    return jsonify({
        ...
        "message": f"Se resolvieron {len(deleted_serials)} componente(s) autogenerado(s) duplicado(s).",
    })


@bp_stock.route("/api/components/autogenerated_duplicates", methods=["GET"])
def get_autogenerated_duplicates():
    if not check_stock_permission():
        return jsonify({"status": "error", "message": "Acceso denegado: Se requiere permiso de Gestión Stock."}), 403
...
@bp_stock.route("/api/components/autogenerated_duplicates/ignored", methods=["GET"])
def get_ignored_autogenerated_duplicates():
    if not check_stock_permission():
        return jsonify({"status": "error", "message": "Acceso denegado: Se requiere permiso de Gestión Stock."}), 403

    with get_db_connection() as conn:
        components = _load_ignored_autogenerated_duplicates(conn)
...
@bp_stock.route("/api/components/ignore_autogenerated_duplicates", methods=["POST"])
def ignore_autogenerated_duplicates():
    if not check_stock_permission():
        return jsonify({"status": "error", "message": "Acceso denegado: Se requiere permiso de Gestión Stock."}), 403
    ...
    with get_db_connection() as conn:
        candidates = _load_autogenerated_duplicate_candidates(conn)
        candidate_serials = {c.get("serial_number") for c in candidates if c.get("serial_number")}
        serials_to_ignore = [serial for serial in requested_serials if serial in candidate_serials]

        if not serials_to_ignore:
            return jsonify({
                "status": "success",
                "ignored_count": 0,
                "ignored_serials": [],
                "message": "No se detectaron componentes autogenerados duplicados válidos para ignorar.",
            })

        for serial in serials_to_ignore:
            conn.execute(
                """
                UPDATE components
                SET ignore_autogenerated_duplicate = 1,
                    ignored_autogenerated_by = %s,
                    ignored_autogenerated_at = NOW()
    ...
    return jsonify({
        "status": "success",
        "ignored_count": len(serials_to_ignore),
        "ignored_serials": serials_to_ignore,
        "message": f"Se ignoraron {len(serials_to_ignore)} componente(s) autogenerado(s) duplicado(s).",
    })


@bp_stock.route("/api/components/unignore_autogenerated_duplicates", methods=["POST"])
def unignore_autogenerated_duplicates():
    if not check_stock_permission():
        return jsonify({"status": "error", "message": "Acceso denegado: Se requiere permiso de Gestión Stock."}), 403
    ...
    with get_db_connection() as conn:
        ignored_components = _load_ignored_autogenerated_duplicates(conn)
        ignored_serials = {c.get("serial_number") for c in ignored_components if c.get("serial_number")}
        serials_to_restore = [serial for serial in requested_serials if serial in ignored_serials]
        ...
        if not serials_to_restore:
            return jsonify({
                ...
                "restored_count": 0,
                "restored_serials": [],
                "message": "No se detectaron componentes autogenerados ignorados válidos para restaurar.",
            })
        ...
        for serial in serials_to_restore:
            conn.execute(
                """
                UPDATE components
                SET ignore_autogenerated_duplicate = 0,
                    ignored_autogenerated_by = NULL,
                    ignored_autogenerated_at = NULL
                WHERE serial_number = %s
                """,
                (serial,),
            )
    ...
    return jsonify({
        "status": "success",
        "restored_count": len(serials_to_restore),
        "restored_serials": serials_to_restore,
        "message": f"Se restauraron {len(serials_to_restore)} componente(s) autogenerado(s) ignorado(s).",
    })
...
def build_orders_view():
    ...
    filter_query = (request.args.get("q") or "").strip()
    filter_status = (request.args.get("status") or "").strip().lower()
    focus_bo_id = (request.args.get("bo_id") or "").strip()
    try:
        with get_db_connection() as conn:
            ...
            stock_comps_rows = conn.execute(
                ...
                WHERE (status = 'Stock' OR status IS NULL OR lifecycle_status = 'stock')
                  ...
                  AND (status NOT IN ('Reservado', 'Installed', 'Baja', 'En Reparacion'))
                ORDER BY component_type ASC, brand_model ASC
                """
            ).fetchall()
            ...
            if filter_query or filter_status:
                filter_query_lower = filter_query.lower()

                def _matches_build_order(item):
                    if filter_status and (item.get("status") or "").strip().lower() != filter_status:
                        return False
                    if not filter_query_lower:
                        return True
                    searchable = " ".join(
                        str(item.get(key) or "")
                        for key in (
                    ...
                    return filter_query_lower in searchable

                build_orders_list = [item for item in build_orders_list if _matches_build_order(item)]
                validados_count = sum(1 for item in build_orders_list if item.get("validation_status") == "validado")
                pendientes_count = sum(
                    1
                    for item in build_orders_list
                    if item.get("validation_status") != "validado" and item.get("status") != "cancelled"
                )
        ...
        with get_db_connection() as conn:
            rows = conn.execute(
                ...
                WHERE (status = 'Stock' OR status IS NULL)
                  AND (lifecycle_status = 'stock' OR lifecycle_status IS NULL)
                  AND (assigned_pc IS NULL OR assigned_pc = '')
                  AND (assigned_user IS NULL OR assigned_user = '')
                  AND (assigned_fuero IS NULL OR assigned_fuero = '')
                  AND (build_order_id IS NULL OR build_order_id = 0)
                ORDER BY component_type ASC, brand_model ASC
                """
            ).fetchall()
        ...
        with get_db_connection() as conn:
            ...
            try:
                relations = conn.execute(
                    """
                    SELECT pnp.pc_name, np.serial_number, np.id as printer_id 
                    FROM network_printers np
                    LEFT JOIN pc_network_printers pnp ON pnp.printer_id = np.id
                    """
                ).fetchall()
                for rel in relations:
                    ...
                    if sn_key:
                        ...
                        net_prn_ids[sn_key] = rel["printer_id"]
            except Exception as e:
            ...
            for r in rows:
                ...
                if item.get("component_type", "").upper() == "IMPRESORA" and item.get("is_network_printer"):
                    ...
                    item["network_printer_id"] = net_prn_ids.get(sn, None)
...
def add_component():
    try:
        ...
        with get_db_connection() as conn:
            ...
            status, lifecycle_status = assignment_component_state(
                assigned_user=assigned_user or None,
                assigned_fuero=assigned_fuero or None,
            )
            added_serials = []
...
def retire_component():
    try:
        ...
        with get_db_connection() as conn:
            ...
            retired_status, retired_lifecycle = retired_component_state()
            conn.execute(
                "UPDATE components SET status = %s, lifecycle_status = %s, build_order_id = NULL, assigned_pc = NULL, assigned_user = NULL, assigned_fuero = NULL WHERE serial_number = %s",
                (retired_status, retired_lifecycle, serial)
            )
            try:
...
def restore_component():
    try:
        ...
        with get_db_connection() as conn:
            ...
            conn.execute(
                "UPDATE components SET status = %s, lifecycle_status = %s, assigned_pc = NULL, assigned_user = NULL, assigned_fuero = NULL WHERE serial_number = %s",
                (stock_status, stock_lifecycle, serial)
            )
                ...
                for serial in clean_serials:
                    ...
                    if comp:
                        conn.execute(
                            """
                            UPDATE components
                            SET status = 'Stock', kit_name = %s, assigned_pc = NULL, assigned_user = NULL, assigned_fuero = NULL, lifecycle_status = 'stock'
                            WHERE serial_number = %s
                            """,
            ...
            for serial in clean_serials:
                ...
                if comp:
                    ...
                    conn.execute(
                        """
                        UPDATE components
                        SET status = %s, assigned_pc = %s, assigned_user = %s, assigned_fuero = %s, kit_name = NULL, lifecycle_status = %s
                        WHERE serial_number = %s
                        """,
                        (new_status, pc_name or None, assigned_user or None, assigned_fuero or None, new_lifecycle, serial)
                    )
            ...
            for serial in remove_serials:
                ...
                if comp:
                    conn.execute(
                        """
                        UPDATE components
                        SET status = 'Stock', kit_name = NULL, lifecycle_status = 'stock'
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            conn.execute(
                ...
                WHERE kit_name = %s
                """,
                (new_status, pc_name or None, assigned_user or None, assigned_fuero or None, new_lifecycle, kit_name)
            )
    ...
    try:
        ...
        with get_db_connection() as conn:
            conn.execute(
                """
                UPDATE components
                SET status = 'Stock', lifecycle_status = 'stock', kit_name = NULL
                WHERE kit_name = %s
                """,
                (kit_name,)
            )
...
def swap_failing_component():
    try:
        ...
        with get_db_connection() as conn:
            ...
            retired_status, retired_lifecycle = retired_component_state(scrap=True)
            conn.execute(
                """
                UPDATE components
                SET status = %s,
                ...
                WHERE serial_number = %s
                """,
                (retired_status, retired_lifecycle, old_serial)
            )
            ...
            if new_serial:
                ...
                conn.execute(
                    ...
                    (
                        new_status,
                        target_pc if target_pc != 'Desconocido' else None,
                        user_target,
                        fuero_target,
                        new_lifecycle,
                        new_serial,
            ...
            for item in items:
                ...
                if comp:
                    released = conn.execute(
                        """
                        UPDATE components
                        SET status = 'Stock', lifecycle_status = 'stock', assigned_pc = NULL, assigned_user = NULL,
